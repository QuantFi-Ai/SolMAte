...
def test_discovery_system():
    """Test the discovery system and identify issues"""
    print("🧪 SolM8 Discovery System Test")
    print_separator()
    ...
    if not discovery_working or not ai_recommendations_working:
        print("\n🔴 ISSUE CONFIRMED: Users cannot see each other in the discovery feed.")
        print("\nROOT CAUSE:")
        print("The MongoDB queries in both the discover_users and get_ai_recommendations endpoints")
        print("don't include any sorting, so results are returned in insertion order (oldest first).")
        print("Since the default limit is 10, only the oldest 10 users are returned.")
        print("Newly created users don't appear in discovery because they're not in the first 10 users.")
